...
- Minikube является локальной учебной средой, а не production-кластером;
- SQLite подходит для демонстрации, но не для высокой конкурентной нагрузки;
- файловое хранилище MLflow не является рекомендуемым production-backend;
- строгий concept drift нельзя подтвердить без поступающих фактических меток;
- автоматическое переобучение требует дополнительной проверки качества новой модели перед её публикацией.
